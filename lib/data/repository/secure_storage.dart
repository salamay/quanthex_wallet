...
import 'package:quanthex/data/Models/assets/custom_token.dart' show CustomToken, customTokenToJson, customTokenFromJson;
...
class SecureStorage {
  ...
  Future <void> saveWallet(List<WalletModel> wallets)async{
    ...
    if(data!=null&&data.isNotEmpty){
      ...
      existingWallets=decodedData.map((e) => WalletModel.fromJson(e)).toList();
      for(WalletModel wallet in wallets){
        bool exists=existingWallets.any((element) => element.walletAddress==wallet.walletAddress&&element.chainId==wallet.chainId);
        if(!exists){
  ...
  Future<List<WalletModel>> getWallets()async {
    ...
    if (data != null && data.isNotEmpty) {
      List<dynamic> decodedData = json.decode(data);
      List<WalletModel> existingWallets = decodedData.map((e) => WalletModel.fromJson(e)).toList();
  ...
  Future<void> saveToken({required List<CustomToken> customToken})async{
    try{
      await _storage!.write(key: custom_tokens, value: customTokenToJson(customToken));
  ...
  Future<List<CustomToken>> getCustomTokens({required String chain})async{
    try{
      ...
      if(data.isNotEmpty){
        List<CustomToken> tokens= customTokenFromJson(data.toString()).where((e)=>e.chainSymbol!.toLowerCase()==chain.toLowerCase()).toList();
        return tokens;
      }else{
        return [];
      }
